...
class Points():
    ...
    def __init__(self, *vectors, color=black, alpha=1.0):
        self.vectors = list(vectors)  # 向量列表
        self.color = color  # 点的颜色默认black
        self.alpha = alpha  # 透明度，默认1.0（不透明）
...
class Arrow():
    ...
    def __init__(self, tip, tail=(0, 0), color=red, alpha=1.0, linestyle='-'):
        self.tip = tip
        self.tail = tail
        self.color = color
        self.alpha = alpha  # 透明度，默认1.0（不透明）
        self.linestyle = linestyle  # 线型: '-' 实线, '--' 虚线, ':' 点线, '-.' 点划线
...
def draw(*objects, origin=True, axes=True, grid=(1, 1), nice_aspect_ratio=True, width=6, save_as=None):
    """
    draw 函数是一个统一的绘图入口——给它若干几何对象（Polygon, Points, Arrow, Segment），
    它自动计算合适坐标范围、添加网格/坐标轴/原点、按类型绘制，并可保存图片。

    *objects: 传入的对象，可以是Polygon, Points, Arrow, Segment
    origin=True: 是否绘制原点(0,0),并且再原点处绘制一个黑色的x
    axes=True: 是否绘制坐标轴(粗黑线)
    grid=(1, 1): 是否绘制网格,以及网格的间距,例如(1,1)表示x轴和y轴的网格间距均为1
                 设置为None或者False表示不绘制网格
    nice_aspect_ratio=True: 让图像窗口的宽高比与坐标区域的宽高比一致，避免变形。
    width=6: 图像的宽度，单位为英寸，如果nice_aspect_ratio=True, 则高度会自动调整以保持宽高比
    save_as: 若提供文件路径（如 'fig.png'），会在 plt.show() 前保存图像。
    """
    ...
    for object in objects:
        if type(object) == Polygon:
            for i in range(0, len(object.vertices)):
                ...
                plt.plot([x1, x2], [y1, y2], color=object.color, alpha=object.alpha)
            if object.fill:
            ...
            plt.scatter(xs, ys, color=object.color, alpha=object.alpha)
        elif type(object) == Arrow:
            ...
            plt.gca().arrow(tail[0], tail[1], new_x, new_y,
                            ...
                            fc=object.color, ec=object.color, alpha=object.alpha,
